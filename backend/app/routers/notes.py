...
async def list_notes(auth: AuthContext = Depends(get_auth)):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM notes WHERE user_id = $1 AND organization_id = $2 "
...
async def create_note(note: NoteCreate, auth: AuthContext = Depends(get_auth)):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "INSERT INTO notes (id, user_id, organization_id, title, content, source, tags, pinned, created_at, updated_at) "
            "VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, $9)",
            note_id, auth.user_id, auth.org_id,
            note.title, note.content, note.source,
            note.tags or [], note.pinned, now,
        )
...
async def update_note(note_id: str, updates: NoteUpdate, auth: AuthContext = Depends(get_auth)):
    ...
    async with pool.acquire() as conn:
        existing = await conn.fetchrow(
            "SELECT * FROM notes WHERE id = $1 AND user_id = $2 AND organization_id = $3",
            note_id, auth.user_id, auth.org_id,
        )
        if not existing:
        ...
        await conn.execute(
            f'UPDATE notes SET {", ".join(sets)} WHERE id = ${idx}', *params
        )
        row = await conn.fetchrow("SELECT * FROM notes WHERE id = $1", note_id)
        return row_to_dict(row)
...
async def delete_note(note_id: str, auth: AuthContext = Depends(get_auth)):
    ...
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM notes WHERE id = $1 AND user_id = $2 AND organization_id = $3",
            note_id, auth.user_id, auth.org_id,
        )
        if result == "DELETE 0":
            raise HTTPException(404, "Note not found")

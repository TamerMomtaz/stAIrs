...
import { BORDER, GRAD_ACCENT, INK_ON_ACCENT, glass } from "../constants";
    ...
    <div className="flex h-[calc(100vh-180px)] gap-3">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pb-4 px-1 min-h-0">
          ...
          {messages.map((m,i) => m.failure ? <div key={i} className="flex justify-start"><div className="max-w-[74ch] w-full"><AiUnavailable kind={m.failure} lang={lang} onRetry={() => retrySend(i)} retrying={loading} /></div></div> : <div key={i} className={`flex ${m.role==="user"?"justify-end":"justify-start"}`}><div className={`group/msg relative max-w-[74ch] px-[18px] py-[15px] rounded-2xl text-[15px] leading-[1.68] ${m.role==="user"?"bg-amber-500/10 text-amber-100 rounded-br-md":m.error?"bg-red-500/10 text-red-300 rounded-bl-md border border-red-500/20":"bg-[#162544] text-gray-200 rounded-bl-md border border-[#1e3a5f]"}`}>{m.role==="ai"?<><Markdown text={m.text} onMatrixClick={onMatrixClick}/><LoadMatrixButtons text={m.text} onLoadMatrix={onMatrixClick}/></>:<div className="whitespace-pre-wrap">{m.text}</div>}{m.role==="ai"&&!m.error&&<><SourcesUsed sources={m.sources_used} />{m.validation && <ValidationWarnings validation={m.validation} />}<div className="flex items-center gap-2 mt-2 flex-wrap">{m.validation && <ConfidenceBadge validation={m.validation} agentsUsed={m.agents_used} />}{showTelemetry&&m.provider_display&&<span className="text-[10px] text-gray-500 bg-sunken px-1.5 py-0.5 rounded">{"\u26A1"} {m.provider_display}</span>}{showTelemetry&&m.tokens>0&&<span className="text-[10px] text-gray-600">{m.tokens} tokens</span>}{showTelemetry&&m.agents_used&&m.agents_used.length>0&&<span className="text-[10px] text-gray-600">{m.agents_used.length} agent{m.agents_used.length!==1?"s":""}</span>}<div className="flex-1"/>{onSaveNote&&<button onClick={() => onSaveNote(m.text.slice(0,60), m.text, "ai_chat")} className="opacity-0 group-hover/msg:opacity-100 text-[10px] text-gray-600 hover:text-amber-400 transition px-1.5 py-0.5 rounded hover:bg-amber-500/10" title="Save to Notes">{"\uD83D\uDCCC"} Save</button>}</div></>}</div></div>)}
        ...
        <div className="shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={send} disabled={loading||!input.trim()} className="px-5 py-3 rounded-xl font-medium text-sm disabled:opacity-30 transition-all hover:scale-105 self-end" style={{ background:GRAD_ACCENT, color:INK_ON_ACCENT }}>{isAr?"إرسال":"Send"}</button>
          </div>
        </div>
      </div>
    </div>

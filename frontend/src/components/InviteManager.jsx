  ...
  return (
    <div className="rounded-xl p-4" style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.30)" }}>
      ...
      <div className="flex gap-2 mb-2">
        ...
        <button onClick={() => copy(link)} className="px-3 py-2 rounded-lg text-xs font-semibold shrink-0" style={{ background: `linear-gradient(135deg, ${GOLD}, ${GOLD_L})`, color: DEEP }}>
          {copied ? (isAr ? "تم النسخ" : "Copied") : (isAr ? "نسخ الرابط" : "Copy link")}
        </button>
      </div>
      <button onClick={() => copy(invite.token)} className="text-[11px] text-gray-500 hover:text-amber-400 transition underline">
        {isAr ? "انسخ الرمز فقط" : "Copy just the code"}
      </button>
      <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${BORDER}` }}>
        <button onClick={onDone} className="text-xs text-gray-400 hover:text-white transition">
          {isAr ? "تم" : "Done"}
        </button>
      </div>
    </div>
...
export const InviteManager = ({ open, onClose, lang, currentUserRole }) => {
  ...
  const statusOf = (i) => {
    if (i.revoked_at) return { label: isAr ? "ملغاة" : "Revoked", cls: "text-gray-500" };
  ...
  return (
    <Modal open={open} onClose={onClose} title={isAr ? "دعوة زملائك" : "Invite your team"} size="lg">
      {!isAdmin ? (
        <p className="text-sm text-gray-400 py-6 text-center" data-testid="not-admin">
          {isAr
        ...
        <div className="space-y-5">
          ...
          {fresh ? (
            ...
            <div className="rounded-xl p-4" style={glass(0.4)}>
              <div className="text-sm text-white font-medium mb-3">{isAr ? "دعوة جديدة" : "New invitation"}</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-gray-500 block mb-1">{isAr ? "البريد الإلكتروني (اختياري)" : "Email (optional)"}</label>
                ...
                <div>
                  <label className="text-[11px] text-gray-500 block mb-1">{isAr ? "الدور" : "Role"}</label>
                  ...
                  <div className="text-[10px] text-gray-600 mt-1">{ROLES.find(r => r.value === role)?.hint}</div>
                </div>
                <div>
                  <label className="text-[11px] text-gray-500 block mb-1">{isAr ? "تنتهي خلال" : "Expires in"}</label>
                ...
                <div className="flex items-end">
                  <button onClick={create} disabled={creating} className="w-full px-4 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-40 transition hover:scale-[1.01]" style={{ background: `linear-gradient(135deg, ${GOLD}, ${GOLD_L})`, color: DEEP }} data-testid="create-invite">
          ...
          <div>
            <div className="text-sm text-white font-medium mb-2">{isAr ? "الدعوات" : "Invitations"}</div>
            {loadError && <div className="text-xs text-red-400 mb-2" data-testid="load-error">{loadError}</div>}
            {loading ? (
              <div className="text-xs text-gray-600 py-6 text-center">{isAr ? "جاري التحميل..." : "Loading…"}</div>
            ) : invites.length === 0 ? (
              <div className="text-xs text-gray-600 py-6 text-center" data-testid="no-invites">
                ...
                {invites.map(i => {
                  ...
                  return (
                    <div key={i.id} className="flex items-center gap-3 p-2.5 rounded-lg" style={glass(0.3)}>
                      ...
                      {pending && (
                        <button onClick={() => revoke(i.id)} disabled={revoking === i.id} className="text-[10px] text-gray-600 hover:text-red-400 transition px-2 py-1 rounded hover:bg-red-500/10 shrink-0 disabled:opacity-40" data-testid={`revoke-${i.id}`}>

    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", background: `linear-gradient(135deg, ${DEEP} 0%, ${RAISED} 40%, ${RAISED} 70%, ${DEEP_MID} 100%)` }}>
      <div style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "440px", padding: "48px 40px", borderRadius: "16px", backdropFilter: "blur(20px)", background: "rgb(var(--surface-raised-rgb) / 0.85)", border: `1px solid ${tint(GOLD, 20)}`, boxShadow: `0 25px 60px ${cast(0.5)}` }}>
          ...
          <div data-testid="reset-screen">
            {resetDone ? (
              ...
              <div data-testid="reset-invalid">
                ...
                <button className="transition hover:brightness-110 active:brightness-95" type="button" onClick={() => { setResetToken(""); setReset(null); setMode("login"); }} style={{ width: "100%", padding: "12px", borderRadius: "10px", fontSize: "14px", color: INK_3, background: "transparent", border: `1px solid ${tint(GOLD, 15)}`, cursor: "pointer" }}>Back to sign in</button>
              </div>
            ) : (
        ...
        <div style={{ display: "flex", marginBottom: "24px", borderRadius: "10px", overflow: "hidden", border: `1px solid ${tint(GOLD, 15)}` }}>
          <button className="transition hover:brightness-110 active:brightness-95" type="button" onClick={() => switchMode("login")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", background: mode === "login" ? GRAD_ACCENT : "transparent", color: mode === "login" ? DEEP : INK_3, transition: "all 0.2s" }}>Sign In</button>
          <button className="transition hover:brightness-110 active:brightness-95" type="button" onClick={() => switchMode("signup")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", background: mode === "signup" ? GRAD_ACCENT : "transparent", color: mode === "signup" ? DEEP : INK_3, transition: "all 0.2s" }}>Create Account</button>
        </div>
          ...
          <form onSubmit={handleSignup} noValidate>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {showInviteField ? (
                ...
                <button className="transition hover:brightness-110 active:brightness-95" type="button" onClick={() => setShowInviteField(true)} style={{ background: "none", border: "none", color: INK_MUTED, fontSize: "13px", cursor: "pointer", textDecoration: "underline", padding: 0, textAlign: "left" }} data-testid="have-invite-btn">
        ...
        {showForgot && (
          <div style={{ position: "fixed", inset: 0, background: cast(0.6), display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={() => setShowForgot(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: RAISED, border: `1px solid ${tint(GOLD, 20)}`, borderRadius: "12px", padding: "32px", maxWidth: "400px", width: "90%", textAlign: "center" }}>
              ...
              <button className="transition hover:brightness-110 active:brightness-95" onClick={() => setShowForgot(false)} style={{ padding: "10px 24px", borderRadius: "8px", fontWeight: 600, fontSize: "14px", color: DEEP, border: "none", cursor: "pointer", background: GRAD_ACCENT }}>OK</button>
            </div>
          </div>
        )}

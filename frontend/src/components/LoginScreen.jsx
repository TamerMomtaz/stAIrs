    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", background: `linear-gradient(135deg, ${DEEP} 0%, ${RAISED} 40%, ${RAISED} 70%, ${DEEP_MID} 100%)` }}>
      <div style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "440px", padding: "48px 40px", borderRadius: "16px", backdropFilter: "blur(20px)", background: "rgb(var(--surface-raised-rgb) / 0.85)", border: `1px solid ${tint(GOLD, 20)}`, boxShadow: `0 25px 60px ${cast(0.5)}` }}>
          ...
          <div data-testid="reset-screen">
            {resetDone ? (
              <div style={{ textAlign: "center" }}>
                ...
                <button type="button" onClick={() => { setResetToken(""); setReset(null); setResetDone(false); setMode("login"); }} style={{ width: "100%", padding: "14px", borderRadius: "10px", fontWeight: 600, fontSize: "15px", color: INK_ON_ACCENT, border: "none", cursor: "pointer", background: GRAD_ACCENT }}>Go to sign in</button>
              </div>
            ) : reset === null ? (
              ...
              <form onSubmit={submitReset} noValidate>
                ...
                <button type="submit" disabled={busy} style={{ width: "100%", marginTop: "24px", padding: "14px", borderRadius: "10px", fontWeight: 600, fontSize: "16px", color: INK_ON_ACCENT, border: "none", cursor: "pointer", background: GRAD_ACCENT, opacity: busy ? 0.5 : 1 }} data-testid="reset-submit">
        ...
        <div style={{ display: "flex", marginBottom: "24px", borderRadius: "10px", overflow: "hidden", border: `1px solid ${tint(GOLD, 15)}` }}>
          <button type="button" onClick={() => switchMode("login")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", background: mode === "login" ? GRAD_ACCENT : "transparent", color: mode === "login" ? INK_ON_ACCENT : INK_3, transition: "all 0.2s" }}>Sign In</button>
          <button type="button" onClick={() => switchMode("signup")} style={{ flex: 1, padding: "10px", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", background: mode === "signup" ? GRAD_ACCENT : "transparent", color: mode === "signup" ? INK_ON_ACCENT : INK_3, transition: "all 0.2s" }}>Create Account</button>
        </div>
        ...
        {mode === "login" ? (
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" disabled={busy} style={{ width: "100%", marginTop: "24px", padding: "14px", borderRadius: "10px", fontWeight: 600, fontSize: "16px", color: INK_ON_ACCENT, border: "none", cursor: "pointer", background: GRAD_ACCENT, opacity: busy ? 0.5 : 1, transition: "transform 0.2s" }}>{busy ? "..." : "Sign In"}</button>
          ...
          <form onSubmit={handleSignup} noValidate>
            ...
            <button type="submit" data-testid="signup-submit" disabled={busy || checkingInvite} style={{ width: "100%", marginTop: "24px", padding: "14px", borderRadius: "10px", fontWeight: 600, fontSize: "16px", color: INK_ON_ACCENT, border: "none", cursor: "pointer", background: GRAD_ACCENT, opacity: (busy || checkingInvite) ? 0.5 : 1, transition: "transform 0.2s" }}>
        ...
        {showForgot && (
          <div style={{ position: "fixed", inset: 0, background: cast(0.6), display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={() => setShowForgot(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: RAISED, border: `1px solid ${tint(GOLD, 20)}`, borderRadius: "12px", padding: "32px", maxWidth: "400px", width: "90%", textAlign: "center" }}>
              ...
              <button onClick={() => setShowForgot(false)} style={{ padding: "10px 24px", borderRadius: "8px", fontWeight: 600, fontSize: "14px", color: INK_ON_ACCENT, border: "none", cursor: "pointer", background: GRAD_ACCENT }}>OK</button>
            </div>
          </div>
        )}

...
export const StairEditor = ({ open, onClose, stair, allStairs, onSave, onDelete, lang }) => {
  ...
  const doSave = async () => { if (!form.title.trim()) return; setSaving(true); try { await onSave(isNew ? { ...form, sort_order: computeSortOrder() } : form, stair?.id); onClose(); } catch(e) { alert(e.message); } setSaving(false); };

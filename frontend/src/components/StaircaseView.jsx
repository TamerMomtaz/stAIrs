    ...
    return (
      <div key={s.id} style={{ marginLeft: depth*24 }}>
        <div className={`group rounded-xl my-1.5 transition-all ${isExp?"ring-1":""}`} style={{ borderLeft:`3px solid ${color}`, ...(isExp?{ringColor:`${tint(color, 25)}`,background:"rgb(var(--surface-raised-rgb) / 0.4)"}:{}) }}>
          <div className="flex items-center gap-2 p-3 cursor-pointer hover:bg-lift/[0.03] rounded-xl transition focus:outline-none focus:ring-2 focus:ring-accent/40" {...clickable(() => setExpanded(prev => { const next = prev===s.id?null:s.id; if (next===s.id) fireGuidance("stair_expanded", { name: isAr&&s.title_ar?s.title_ar:s.title }); return next; }), { label: isAr&&s.title_ar?s.title_ar:s.title })}>
